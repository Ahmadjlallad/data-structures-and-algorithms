...
an array of numbers and returns a new array of each of those
  numbers raised to the 3rd power (hint: look up Math.pow()).
...
, given an array of strings,
 uses map to return a new array containing each string followed by
...
const forLoopTwoToThe = (arr) => {
  ...
  for (const iterator of arr) {
    localArr.push(Math.pow(2, iterator));
  }
  return localArr;
};
...
forLoopTwoToThe function from challenge 4 and your forEachTwoToThe function from challenge 5,
...
const charCode = (arr) => {
  // Solution code here...
  return arr.map((chr) => chr.charCodeAt(0));
};
...
const evenOdd = (arr) => {
  // Solution code here...
  return arr.map((numOrIsIt) => {
    if (!(typeof numOrIsIt === "number")) return "N/A";
    return numOrIsIt % 2 === 0 ? "even" : "odd";
  });
};
...
const extractAbilities = (arr) => {
  // Solution code here..
  return arr.map(({ ability: { name } }) => name);
};
...
Write a function named extractStats that,
given an array of stats, uses map to return an array of
objects containing the stat name and the total.
...
const extractStats = (arr) => {
  // Solution code here...
  return arr.map(({ stat: { name }, effort, baseStat }) => {
    return { name, total: effort + baseStat };
  });
};
