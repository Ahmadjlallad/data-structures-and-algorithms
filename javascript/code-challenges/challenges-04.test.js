...
This function should first create a new array. Then iterate over the
input array and modify each value based on the callback function provided.
...
const updateAnimal = (arr, callback) => {
  // Solution code here...
  const newArr = arr;
  return newArr.sort((a, b) => (a > b ? 1 : -1)).map(callback);
};
...
each of which has a 'price' property, and sorts those objects by price, lowest to highest,
...
const sortByPrice = (arr) => {
  // Solution code here...
  return arr.sort(({ price: priceA }, { price: priceB }) => priceA - priceB);
};
...
Write a function named sortNumbersByLength that takes in an array of numbers
and sorts those numbers by their length.
...
const sortPeople = (arr) => {
  // Solution code here...
  const arrCopy = [...arr];
  return arrCopy.sort((a, b) => (a.lastName > b.lastName ? 1 : -1));
};
...
each of which represents a meeting happening a particular day of the week,
with a particular start time and end time.

Sort the meetings by the day on which they happen, Monday-Friday.
It does not matter which order meetings come in on a particular day. For example,
...
const sortMeetingsByDay = (arr) => {
  ...
  const arrCopy = [...arr];
  return arrCopy.sort((a, b) => {
    return list.indexOf(a.dayOfWeek.slice(0, 3)) >
      list.indexOf(b.dayOfWeek.slice(0, 3))
      ? 1
      : -1;
  });
};
...
Sort the meetings in the order that they start.
If two meetings start at the same time on the same day,
the shorter meeting should come first.

You DO NOT need to use your solution to Challenge 9 in completing
Challenge 10.
------------------------------------------------------------------------------------------------ */

const sortSchedule = (arr) => {
  // Solution code here...
  // * I have to use CHALLENGE 12 solution code here
  // * because I didn't mutate the original array in Challenge 12
  const arrCopy = sortMeetingsByDay([...arr]);

  return arrCopy.sort((a, b) => {
    if (a.dayOfWeek === b.dayOfWeek) {
      if (Number(a.start) === Number(b.start))
        return Number(a.end) - Number(b.end);
      else return Number(a.start) - Number(b.start);
    } else return a;
  });
};
